...
tokenizer = AutoTokenizer.from_pretrained(BERT_MODEL_PERTRAINED_PATH)   # 指定的路径加载预训练的BERT tokenizer，它用于将文本转换为模型可以理解的token ID
...
def model_for_bert(request_text: Union[str, List[str]]) -> Union[str, List[str]]:   # 函数定义：接受文本或文本列表
    ...
    classify_result = [CATEGORY_NAME[x] for x in pred]  # 将数字预测结果转换为对应的类别名称
    return classify_result[0] if len(classify_result) == 1 else classify_result
