...
mod tests {
    ...
    #[test]
    fn abi_prj_dispatch_ioctl_codes() {
        assert_eq!(BB_GET_PRJ_DISPATCH, 0x010000C8);
        assert_eq!(BB_SET_PRJ_DISPATCH, 0x020000C8);
    }

    #[test]
    fn abi_prj_command_ids() {
        assert_eq!(PRJ_CMD_SET_ROLE, 0);
        assert_eq!(PRJ_CMD_SET_AP_MAC, 1);
        assert_eq!(PRJ_CMD_SET_MAC, 11);
        assert_eq!(PRJ_CMD_SET_BAND, 3);
        assert_eq!(PRJ_CMD_SET_RESET_DB, 4);
        assert_eq!(PRJ_CMD_SET_PWR, 5);
        assert_eq!(PRJ_CMD_GET_ROLE, 129);
        assert_eq!(PRJ_CMD_GET_BAND, 128);
        assert_eq!(PRJ_CMD_GET_AP_MAC, 130);
        assert_eq!(PRJ_CMD_GET_RUNSYS, 133);
        assert_eq!(PRJ_CMD_SET_PSRAM_START, 7);
        assert_eq!(PRJ_CMD_SET_PSRAM_DATA, 8);
        assert_eq!(PRJ_CMD_SET_PSRAM_DONE, 9);
    }

    #[test]
    fn abi_prj_dispatch_header_layout() {
        // prj_rpc_hdr_t: cmdid(1) + length(2,u16 LE) + padding(1) + data
        // With PACK it is 4 bytes; without PACK the u16 may add alignment.
        // The hand-written dispatch layer writes cmdid at offset 0,
        // length at offset 1-2 (LE), and payload at offset 4.
        let data_offset: usize = 4;
        assert_eq!(data_offset, 4, "PRJ dispatch payload offset must be 4");
    }

    #[test]
    fn abi_bindgen_prj_struct_sizes() {
        // These values are verified against the latest bindgen output
        // (generated from third_party/include/prj_rpc.h with PACK).
        assert_eq!(std::mem::size_of::<prj_cmd_set_role_t>(), 1);
        assert_eq!(std::mem::size_of::<prj_cmd_set_mac_t>(), BB_MAC_LEN);
        assert_eq!(std::mem::size_of::<prj_cmd_set_slot_mac_t>(), 1 + BB_MAC_LEN);
    }

    #[test]
    fn abi_prj_dispatch_input_buffer_size() {
        assert_eq!(std::mem::size_of::<bb_set_prj_dispatch_in_t>(), 256);
    }

    // ── 核心常量守卫 ──

    #[test]
    fn abi_sdk_core_constants() {
        assert_eq!(BB_MAC_LEN, 4);
        assert_eq!(BB_PLOT_POINT_MAX, 10);
        assert_eq!(BB_BLACK_LIST_SIZE, 3);
        assert_eq!(BB_CFG_PAGE_SIZE, 1024);
        assert_eq!(BB_SLOT_MAX, 8);
        assert_eq!(BB_DATA_USER_MAX, 10);
        assert_eq!(BB_HOT_UPGRADE_DATA_SIZE, BB_CFG_PAGE_SIZE - 8);
        assert_eq!(BB_HOT_UPGRADE_CHUNK_SIZE, 0x300);
        assert_eq!(BB_HOT_UPGRADE_CHUNK_SIZE, 768);
        assert!(BB_HOT_UPGRADE_CHUNK_SIZE <= BB_HOT_UPGRADE_DATA_SIZE);
    }

    #[test]
    fn abi_bb_ioctl_request_macros() {
        assert_eq!(BB_REQ_GET, 1);
        assert_eq!(BB_REQ_SET, 2);
        assert_eq!(BB_GET_STATUS, bb_request(BB_REQ_GET, 0));
        assert_eq!(BB_GET_SYS_INFO, bb_request(BB_REQ_GET, 105));
        assert_eq!(BB_GET_BAND_INFO, bb_request(BB_REQ_GET, 13));
        assert_eq!(BB_GET_CHAN_INFO, bb_request(BB_REQ_GET, 10));
        assert_eq!(BB_GET_MCS_MODE, bb_request(BB_REQ_GET, 117));
        assert_eq!(BB_SET_HOT_UPGRADE_WRITE, bb_request(BB_REQ_SET, 10));
        assert_eq!(BB_SET_HOT_UPGRADE_CRC32, bb_request(BB_REQ_SET, 11));
        assert_eq!(BB_SET_SYS_REBOOT, bb_request(BB_REQ_SET, 14));
    }

    #[test]
    fn abi_bb_role_and_user_constants() {
        assert_eq!(BB_ROLE_AP, 0);
